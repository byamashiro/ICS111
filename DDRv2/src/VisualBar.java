...
public class VisualBar {
	...
	boolean hpChange(int hp) { // changing the health bar relative to the input health points
		if (hp > 0) { // if the hp is greater than 0, continue the health bar functions		
			float healthPercentage = 0f;
			
			if ( hp < 600 ) { // if the health is less than the maximum, start a color gradient 
